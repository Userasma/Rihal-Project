...
func RenderUserListHTML(users []models.Main) string {
	content, err := ioutil.ReadFile("frontend/theuserList.html")
	if err != nil {
		// Handle the error if the file cannot be read
		return "Error reading HTML file"
	}
	...
	tmpl := template.Must(template.New("theuserList.html").Parse(string(content)))
